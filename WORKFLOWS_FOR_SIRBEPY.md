...
2. Smoke test the per-project notifications + sound packs feature:
   ...
   c. Switch Pack dropdown to "Peon (Orc) (not installed)" → Install button appears → click → wait → Peon sounds populate, button hides.
   ...
   f. Toggle Done override on, pick Peon → `work-work.wav`. Fire a hook for that project → "Work work" plays.
