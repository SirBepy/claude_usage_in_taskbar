...
- News AI summary (billed): open a real Anthropic news article in the app; confirm the 2-paragraph Claude summary generates (subscription-billed), reads well ("what shipped + why it matters"), caches on reopen, and Regenerate works. Tinker the prompt in `src-tauri/src/news/summarizer.rs` `build_prompt` if the flavor is off.
