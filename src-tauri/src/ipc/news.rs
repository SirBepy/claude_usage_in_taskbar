...
#[tauri::command]
pub fn list_news() -> Result<Vec<NewsPost>, String> {
...
#[tauri::command]
pub fn mark_news_read(slug: String, app: AppHandle) -> Result<(), String> {
    let path = paths::news_file().map_err(|e| e.to_string())?;
    let mut store = news::load(&path);
    news::mark_read(&mut store, &slug);
    news::save(&path, &store).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn mark_all_news_read(app: AppHandle) -> Result<(), String> {
    let path = paths::news_file().map_err(|e| e.to_string())?;
    let mut store = news::load(&path);
